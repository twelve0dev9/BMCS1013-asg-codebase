...
//    bookingsStruct bookings[], int& booking_count);
//
//void customer_make_payment_simulated();
//
...
//        }
//        else if (c == 5) {
//            customer_make_payment_simulated();
//        }
//        else if (c == 6) {
...
//    cout << "IU Men’s Grooming & Treatment : Styling, Haircuts, Skin Treatments." << endl;
//    cout << "Operating hours: 12PM to 1AM (Mon to Sat)" << endl;
//    cout << "Email: iugrooming@gmail.com | Phone: 012 345 6789" << endl << endl;
//}
//
//void customer_view_services_experts(users expert[], int EXPERT_COUNT,
//    servicesStruct services[], int SERVICE_COUNT) {
...
//    clearscreen();
//
//    const int totalSlotsPerDay = SLOT_COUNT;
...
//    int rowBreak = 0;
//    for (int d = 1; d <= 31; ++d) {
//        bool full = (bookingsPerDay[d] >= totalSlotsPerDay);
...
//    cout << "Select Service:\n";
//    for (int i = 0; i < SERVICE_COUNT; ++i) {
//        cout << "  " << (i + 1) << ". " << services[i].serviceName << " (RM" << services[i].price << ")\n";
//    }
...
//            if (slot_conflict(bookings[k], expert[eidx].name, d, slots[i].label)) { free = false; break; }
//        cout << "  " << (i + 1) << ". " << slots[i].label << (free ? "" : " (booked)") << "\n";
//        ++listed;
//    }
//    int tidx;
//    bool chosen_ok = false;
//    do {
//        tidx = readIntInRange(1, listed) - 1;
//        bool free = true;
//        for (int k = 0; k < booking_count; ++k)
//            if (slot_conflict(bookings[k], expert[eidx].name, d, slots[tidx].label)) { free = false; break; }
//        if (free) {
//            chosen_ok = true;
//        }
//        else {
//            cout << "That slot is already booked. Choose another.\n";
//        }
//    } while (!chosen_ok);
//
//    if (booking_count < 256) {
//        bookings[booking_count++] = { d, slots[tidx].label, services[sidx].serviceName,
//                                     customer[custIndex].name, expert[eidx].name, services[sidx].price };
//        cout << "Appointment booked for " << customer[custIndex].name << " on Dec " << d
//            << " at " << slots[tidx].label << " with " << expert[eidx].name << ".\n";
//    }
//    else {
...
//
//void customer_make_payment_simulated() {
//    cout << "Payment module (simulated):\n"
//        << "Choose method: 1) Card  2) Online Banking  3) E-Wallet\n";
//    readIntInRange(1, 3);
//    cout << "Payment successful. Receipt emailed. (Demo)\n";
//}
//
//void admin_login(users customer[], int customer_count,
//    users expert[], int EXPERT_COUNT,
...
//    bookingsStruct bookings[], int booking_count) {
//    cout << "Sort Options:\n";
//    cout << "1. Sort by Date (asc)\n";
//    cout << "2. Sort by Customer Name (A-Z)\n";
//    int sortChoice = readIntInRange(1, 2);
//
//    int order[256]; // why: avoid heap & match bookings[256] capacity
...
//        if (k == -1) continue;
//
//        if (bookings[i].service == "Consultation") rows[k].hours += 1;
//        else rows[k].hours += 3;
//        rows[k].servicevalue += (int)bookings[i].price;
//    }
//    for (int i = 0; i < 2; ++i) {
//        if (rows[i].hours >= 30) rows[i].bonusPct = 0.50;
//        else if (rows[i].hours >= 25) rows[i].bonusPct = 0.25;
//        else rows[i].bonusPct = 0.0;
//    }
...
//        else if (choice == 2) {
//            string searchTerm;
//            cout << "Enter search term (customer name): ";
//            getline(cin, searchTerm);
//
//            cout << "\nSEARCH RESULTS:\n";
//            cout << "+----+---------------------+-----------------------------+------------+------------+------+\n";

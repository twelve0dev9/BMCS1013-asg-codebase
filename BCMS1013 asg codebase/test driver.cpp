...
struct bookings {
	...
	int book_date; //have sum control structures for telling customer user to input valid date of booking this
...
int main() {
	...
	for (int i = 0; i < 2; ++i) {
		...
		cout << fixed << setprecision(2) << showpoint << appointments.timeslot->timeslotID << endl 
			...
			<< appointments.booking_status << endl << "Dec " << appointments.book_date << " 2025" << endl
	...
	//	cin >> choice_expert;
	//	appointment(choice_expert, services_available, experts, customer_users, appointments_schedule, arrySize_bookings);}
	return 0;
}